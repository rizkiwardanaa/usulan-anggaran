...
def tampilkan_halaman():
    ...
    st.info("Fitur ini dirancang khusus untuk memproduksi file Excel resmi (Rincian Anggaran Biaya) sesuai standar format universitas. Isi data form di bawah, lalu klik 'Buat File RAB'.")
    ...
    df_rab_input = st.data_editor(
        template_rab, num_rows="dynamic", use_container_width=True, hide_index=True,
        column_config={
    ...
    def export_excel_rab(df_items, header_data):
        ...
        meta_rows = [
            ...
            ("Kegiatan:", header_data['kegiatan']), ("Sasaran Kegiatan:", header_data['sasaran']),
            ("Klasifikasi Rincian Output:", header_data['kro']), ("Volume:", header_data['vol']),
            ("Satuan Ukur:", header_data['satuan']), ("Alokasi Dana:", f"Rp. {header_data['alokasi']:,.0f}".replace(',','.'))
        ]
        ...
        for ro, group_ro in df_items.groupby("Rincian Output"):
            ...
            cell_ro1 = ws.cell(row=row_pointer, column=1, value=ro); cell_ro1.font = font_bold; cell_ro1.border = border_all
            ...
            cell_ro5 = ws.cell(row=row_pointer, column=5, value=tot_ro); cell_ro5.font = font_bold; cell_ro5.border = border_all; cell_ro5.number_format = '#,##0'
            row_pointer += 1

            for komp, group_komp in group_ro.groupby("Komponen"):
                tot_komp = group_komp["Jumlah_Biaya"].sum()
                cell_ko1 = ws.cell(row=row_pointer, column=1, value=f"  {komp}"); cell_ko1.font = font_bold; cell_ko1.border = border_all
                ws.cell(row=row_pointer, column=2).border = border_all; ws.cell(row=row_pointer, column=3).border = border_all; ws.cell(row=row_pointer, column=4).border = border_all
                cell_ko5 = ws.cell(row=row_pointer, column=5, value=tot_komp); cell_ko5.font = font_bold; cell_ko5.border = border_all; cell_ko5.number_format = '#,##0'
                row_pointer += 1

                for akun, group_akun in group_komp.groupby("Akun Belanja"):
                    tot_akun = group_akun["Jumlah_Biaya"].sum()
                    cell_ak1 = ws.cell(row=row_pointer, column=1, value=f"    {akun}"); cell_ak1.font = font_bold; cell_ak1.border = border_all
                    ws.cell(row=row_pointer, column=2).border = border_all; ws.cell(row=row_pointer, column=3).border = border_all; ws.cell(row=row_pointer, column=4).border = border_all
                    cell_ak5 = ws.cell(row=row_pointer, column=5, value=tot_akun); cell_ak5.font = font_bold; cell_ak5.border = border_all; cell_ak5.number_format = '#,##0'
                    row_pointer += 1

                    for _, baris_data in group_akun.iterrows():
                        ws.cell(row=row_pointer, column=1, value=f"      - {baris_data['Uraian Belanja (Barang/Jasa)']}").border = border_all
    ...
    if st.button("🚀 Buat File RAB (Excel)", type="primary"):
        ...
        if valid_rab_input.empty:
            ...
            header_metadata = {"kegiatan": rab_kegiatan, "sasaran": rab_sasaran, "kro": rab_kro, "vol": rab_vol, "satuan": rab_satuan, "alokasi": rab_alokasi}
